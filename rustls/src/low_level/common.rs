...
impl LlConnectionCommon {
    ...
    pub(super) fn encode_tls_data(
        ...
        // Take the state, return an error if the state was already taken.
        let Some(taken_after_encode) = after_encode.take() else {
            return Err(EncodeError::AlreadyEncoded);
        };
        // Try to encode the generated message. This only fails if `outgoing_tls` is too small to
        // hold the message.
        let encode_result = self.encode_plain_msg(plain_msg, *needs_encryption, outgoing_tls);

        match encode_result {
            Ok(written_bytes) => {
                // If successful, transition to the next state.
                self.state = ConnectionState::AfterEncode(Box::new(taken_after_encode));
                Ok(written_bytes)
            }
            Err(err) => {
                // Restore the state on failure so the aplication layer can call
                // [`MustEncodeTlsData::encode`] again.
    ...
    pub(super) fn next_app_data_record<'i>(
        ...
        // Keep the initial offset so we can decrypt the app data in-place.
        let offset = self.offset;

        let msg = Ok(()).and_then(|()| {
            // Read a message, we don't call [`Self::read_message`] to avoid changing the `offset`
            // field if we read a message that is not an application data record.
            let mut reader = Reader::init(&incoming_tls[self.offset..]);
            let m = OpaqueMessage::read(&mut reader).map_err(|err| match err {
                MessageError::TooShortForHeader | MessageError::TooShortForLength => {
                    InvalidMessage::MessageTooShort
            ...
            if let ContentType::ApplicationData = m.typ {
                // We read an application data record, update `offset`.
                self.offset += reader.used();
                ...
                let Message {
                    ...
                    // This cannot panic because the content type was `ApplicationData` already.
                    unreachable!()
                };

                // Write the decrypted payload into `incoming_tls`
                ...
                Ok(Some(AppDataRecord {
                    discard: self
                        .offset
                        .try_into()
                        .expect("We read a non-empty message"),
                    payload: slice,
                }))
            } else {
                Ok(None)
    ...
    pub(super) fn peek_app_data_record_len<'i>(
        ...
        // FIXME: There is a less wasteful way of doing this. We can read the header and check the
        // content type and payload length without reading the whole payload.

        // We read an opaque message and check its content type to know if it is an application
        // data record.
        let mut reader = Reader::init(&incoming_tls[self.offset..]);
...
/// The state of the connection when emitting a fatal alert.
struct EmitAlert {
...
/// Represents a message generated by [`EmitState::generate_message`].
pub(crate) struct GeneratedMessage {
    /// The generated message
    plain_msg: PlainMessage,
    /// Whether this message should be encrypted or not when encoded.
    needs_encryption: bool,
    /// This field keeps the state that we transition to after
    /// [`LlConnectionCommon::encode_tls_data`] returns successfully. It is taken from this field
    /// after this to prevent repeated calls.
    after_encode: Option<ConnectionState>,
}

impl GeneratedMessage {
    /// Create a new generated message with the state that the connection must have after encoding
    /// it.
    ...
    /// Mark the message so it is encrypted when being encoded.
...
/// Log the read or emitted message for debugging purposes.
